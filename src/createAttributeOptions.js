...
const { createOption } = require('./operations/attributes');

const execute = async () => {
    const start = process.hrtime();

    const rows = await parseCsvFile(
        path.resolve(__dirname, '../data/create-attribute-options.csv'),
    );
    console.log('Attribute options to create:', rows.length);

    await runAttributeOptions(rows, (row) =>
        createOption(row.attribute_code, row.sort_order, row.label, row.front_label, row.value),
    );
