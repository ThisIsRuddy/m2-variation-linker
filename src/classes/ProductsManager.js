...
class ProductsManager {
    ...
    async _getBulkPayload(parentsSkus) {
        return parentsSkus.map(ps => ({
            product: {
                sku: ps, type_id: "configurable"
            }
        }));
    }

    async _convertProducts(parentsSkus) {
        const uri = `/products`;
        const payload = await this._getBulkPayload(parentsSkus);
    ...
    async convertItems(relationships) {
        const start = process.hrtime();

        const parentsSkus = relationships.map(r => r.parentSku);
        console.log("Converting parent products to 'configurables': ", parentsSkus.length);

        const bulk_uuid = await this._convertProducts(parentsSkus);
        console.log(`Bulk convert configurables request sent - took ${secondsElapsedSince(start)}s.`);
